                ...
                <div>
                    ...
                    <div class="d-flex flex-wrap mt-2" style="gap: 10px;">
                        @if (!$data->trash)
                            ...
                            <a href="{{ route('paymentTypeUpdate', ['data' => $data->id]) }}" class="btn d-block btn-outline-primary">Edit</a>
                            @endif

                            @if (Auth::user()->can('paymentType.delete'))
                            <a href="{{ route('paymentTypeRemove', ['data' => $data->id]) }}" class="btn d-block btn-outline-danger delete-confirm">Delete</a>
                            @endif
                        @else
                            @if (Auth::user()->can('paymentType.delete'))
                                <a href="{{ route('paymentTypeTrashRecordsRestore', ['data' => $data->id]) }}" class="btn d-block btn-outline-success restore-confirm">Restore Payment Type</a>
                            @endif
                        @endif
                    </div>
                </div>
...
@if (Auth::user()->can('paymentType.history'))
<header class="body-header">
    <div class="container">
        <div class="header-title">
            <h3>Payment Type's Data Activity Log</h3>
        </div>
    </div>
</header>
@endif
                ...
                <tr>
                    ...
                    <td style="text-align: right;">
                        <span class="table-action">
                            <a href="{{ route('paymentTypeHistoryItemRemove', ['data' => $data->id, 'item' => $value->id]) }}" title="Delete" class='delete-confirm'>
