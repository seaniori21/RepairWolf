                ...
                <div>
                    <div class="d-flex flex-wrap">
                        <span class="profile-name mb-1 fs-5 fw-bolder text-dark d-flex align-items-stretch me-3" style="flex-direction: column;">
                            <small class="fs-6" style="color: #aaa;">Payment Type</small>
                            <span>{{ ucwords($data->paymentType ? $data->paymentType->name : '') }}</span>
                        </span>

                        <span class="profile-name mb-1 fs-5 fw-bolder text-dark d-flex align-items-stretch me-3" style="flex-direction: column;">
                            <small class="fs-6" style="color: #aaa;">Order No</small>
                            @if($data->order)
                            @if (Auth::user()->can('order.view'))
                            <a href="{{ route('admin.order.view', ['data' => $data->order->id]) }}" class="ps-0 d-flex align-items-center relative" target="_blank">
                                ...
                                <span class="ms-1">{{ $data->order->no }}</span>
                            </a>
                            @else
                                {{ $data->order->no }}
                            @endif
                    ...
                    <div class="d-flex flex-wrap">
                        <span class="profile-name mt-2 mb-1 fs-5 fw-bold text-muted d-flex align-items-stretch me-3" style="flex-direction: column;">
                            <small class="fs-6" style="color: #aaa;">Payment Amount</small>
                    ...
                    <div class="d-flex flex-wrap">
                        <span class="profile-name mt-2 mb-1 fs-5 fw-bold text-muted d-flex align-items-stretch me-3" style="flex-direction: column;">
                            <small class="fs-6" style="color: #aaa;">Authorization Approval Code</small>
                            {{ $data->authorization_approval_code ? $data->authorization_approval_code : 'N/A' }}
                        </span>

                        <span class="profile-name mt-2 mb-1 fs-5 fw-bold text-muted d-flex align-items-stretch me-3" style="flex-direction: column;">
                            <small class="fs-6" style="color: #aaa;">Credit Card Number</small>
                            {{ $data->credit_card_number ? $data->credit_card_number : 'N/A' }}
                        </span>
                    </div>

                    <div class="d-flex flex-wrap">
                        <span class="profile-name mt-2 mb-1 fs-5 fw-bold text-muted d-flex align-items-stretch me-3" style="flex-direction: column;">
                            <small class="fs-6" style="color: #aaa;">Expiration Date</small>
                            {{ $data->expiration_date ? $data->expiration_date : 'N/A' }}
                        </span>

                        <span class="profile-name mt-2 mb-1 fs-5 fw-bold text-muted d-flex align-items-stretch me-3" style="flex-direction: column;">
                            <small class="fs-6" style="color: #aaa;">Security Code</small>
                            {{ $data->security_code ? $data->security_code : 'N/A' }}
                        </span>
                    </div>
                    ...
                    <div class="d-flex flex-wrap mt-2" style="gap: 10px;">
                        @if (!$data->trash)
                            ...
                            <a href="{{ route('paymentUpdate', ['data' => $data->id]) }}" class="btn d-block btn-outline-primary">Edit</a>
                            @endif

                            @if (Auth::user()->can('payment.delete'))
                            <a href="{{ route('paymentRemove', ['data' => $data->id]) }}" class="btn d-block btn-outline-danger delete-confirm">Delete</a>
                            @endif
                        @else
                            @if (Auth::user()->can('payment.delete'))
                            <a href="{{ route('paymentTrashRecordsRestore', ['data' => $data->id]) }}" class="btn d-block btn-outline-success restore-confirm">Restore Data</a>
                            @endif
                        @endif
                    </div>
                </div>
...
@if (Auth::user()->can('payment.history'))
<header class="body-header">
    <div class="container">
        <div class="header-title">
                ...
                <tr>
                    ...
                    <td style="text-align: right;">
                        <span class="table-action">
                            <a href="{{ route('paymentHistoryItemRemove', ['data' => $data->id, 'item' => $value->id]) }}" title="Delete" class='delete-confirm'>
