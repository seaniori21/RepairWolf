...
<header class="body-header">
    <div class="container">
        <div class="header-title">
            <h3>Allowed Ip Address details</h3>
        </div>
    </div>
</header>
            ...
            <div>
                <div>
                    <div class="d-flex flex-wrap">
                        <h3 class="profile-name mt-0 mb-1 fw-bolder text-dark d-flex align-items-stretch me-3" style="flex-direction: column;">
                            <small class="fs-6" style="color: #aaa;">IP Address</small>
                            <span>{{ $data->ip_address }}</span>
                        </h3>

                        <h3 class="profile-name mt-0 mb-1 fw-bolder text-dark d-flex align-items-stretch me-3" style="flex-direction: column;">
                            <small class="fs-6" style="color: #aaa;">Note</small>
                            <span>{{ $data->note }}</span>
                        </h3>
                    </div>
                    ...
                    <div class="d-flex flex-wrap mt-2" style="gap: 10px;">
                        @if(!$data->trash)
                            ...
                            <a href="{{ route('ipAllowListUpdate', ['data' => $data->id]) }}" class="btn d-block btn-outline-primary">Edit</a>
                            @endif

                            @if (Auth::user()->can('ipallowlist.delete'))
                            <a href="{{ route('ipAllowListRemove', ['data' => $data->id]) }}" class="btn d-block btn-outline-danger delete-confirm">Delete</a>
                            @endif
                        @else
                            @if (Auth::user()->can('order.delete'))
                            <a href="{{ route('ipAllowListTrashRecordsRestore', ['data' => $data->id]) }}" class="btn d-block btn-outline-success restore-confirm">Restore Data</a>
                            @endif
                        @endif                        
                    </div>
                </div>
...
@if (Auth::user()->can('ipallowlist.history'))
<header class="body-header">
    <div class="container">
        <div class="header-title">
            <h3>Allowed Ip Address's Data Activity Log</h3>
        </div>
    </div>
</header>
                ...
                <tr>
                    ...
                    <td style="text-align: right;">
                        <span class="table-action">
                            <a href="{{ route('ipAllowListHistoryItemRemove', ['data' => $data->id, 'item' => $value->id]) }}" title="Delete" class='delete-confirm'>
